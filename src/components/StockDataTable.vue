  ...
  <div class="stock-data-table">
    ...
    <div class="statistics">
      <el-row :gutter="16">
        ...
        <el-col :span="6">
          <el-statistic title="上涨家数" :value="upCount" />
        </el-col>
        <el-col :span="6">
          <el-statistic title="下跌家数" :value="downCount" />
        </el-col>
      </el-row>
    </div>
    ...
    <el-dialog
      ...
      :title="`股票详情 - ${selectedStock?.name} (${selectedStock?.code})`"
      width="600px"
    >
      <div v-if="selectedStock" class="stock-detail">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="最新价">
            <span :class="getPriceClass(selectedStock.chg_rate)">
          ...
          <el-descriptions-item label="涨跌幅">
            <el-tag :type="getChangeRateTagType(selectedStock.chg_rate)">
              {{ selectedStock.chg_rate?.toFixed(2) }}%
            </el-tag>
          </el-descriptions-item>
          ...
          <el-descriptions-item label="昨收价">
            {{ selectedStock.prev_close?.toFixed(2) }}
          </el-descriptions-item>
...
import type { StockData } from '@/services/dataService'
...
const filteredData = computed(() => {
  ...
  if (sortBy.value) {
    result.sort((a, b) => {
      let valueA = a[sortBy.value as keyof StockData]
      let valueB = b[sortBy.value as keyof StockData]
...
const averageChangeRate = computed(() => {
  ...
  const total = filteredData.value.reduce((sum, stock) => sum + (stock.chg_rate || 0), 0)
  return total / filteredData.value.length
})

const upCount = computed(() => {
  return filteredData.value.filter(stock => (stock.chg_rate || 0) > 0).length
})
...
const showStockDetail = (stock: StockData) => {
